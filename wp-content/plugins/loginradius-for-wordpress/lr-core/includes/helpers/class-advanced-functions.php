...
if ( !class_exists( 'LR_Advanced_Functions' ) ) {
	...
	class LR_Advanced_Functions {
		...
		public function login_radius_print_mappable_fields_checkboxes( $surveygizmo = true ){
			...
			$mappingFields   = login_radius_get_mapping_fields();
			...
			foreach ( $mappingFields as $field ){
				...
				if ( isset( $fieldParts[1] ) ){
					...
					$fieldParts2 = array_map( function( $element ){ return ucfirst( $element ); }, $fieldParts2 );
		...
		public static function login_radius_get_mapping_fields(){
			...
			$mappingFields = array( 'User ID', 'Username', 'First Name', 'Last Name', 'Nicename', 'Email', 'Profile Url', 'Registration Date', 'Display Name', 'Bio' );

			// if basic profile data table exists
			if ( $wpdb->get_var( "SHOW TABLES LIKE '".$wpdb->base_prefix."loginradius_basic_profile_data'" ) == $wpdb->base_prefix.'loginradius_basic_profile_data' ){
				$basicProfileColumns = $wpdb->get_col( 'SHOW COLUMNS FROM '.$wpdb->base_prefix.'loginradius_basic_profile_data' );
			...
			if ( $wpdb->get_var( "SHOW TABLES LIKE '".$wpdb->base_prefix."loginradius_extended_location_data'" ) == $wpdb->base_prefix.'loginradius_extended_location_data' ){
				$extendedLocationColumns = $wpdb->get_col( 'SHOW COLUMNS FROM '.$wpdb->base_prefix.'loginradius_extended_location_data' );
			...
			if ( $wpdb->get_var( "SHOW TABLES LIKE '".$wpdb->base_prefix."loginradius_extended_profile_data'" ) == $wpdb->base_prefix.'loginradius_extended_profile_data' ){
				$mappingFields[] = 'Company';
				$extendedProfileColumns = $wpdb->get_col( 'SHOW COLUMNS FROM '.$wpdb->base_prefix.'loginradius_extended_profile_data' );
